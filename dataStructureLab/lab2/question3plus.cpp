...
Status printStack(ParkStack P);

Status printQueue(WaitQueue W);

Status isK(ParkStack P,int &location);

Status insertK(ParkStack &P,int location,Data data);
...
Status printStack(ParkStack P) {
	...
	while (q < P.top) {
		cout<<q->license<<"号车"<<q->ad<<"在停车场"<<q->site<<"号位置"<<endl;
		q++;
	}
	return OK;
}
...
Status printQueue(WaitQueue W) {
	...
	while (p <= W.rear) {
		cout<<p->data.ad<<"号车"<<"在便道的"<<p->data.site<<"号位置"<<endl;
		p++;
	}
	return OK;
}
...
Status processIn(ParkStack &P, WaitQueue &W, Data &data, int n) {
	...
	if(location!=0) {
		insertK(P,location,data);
	}else if (P.top - P.base < n) {
		push(P, data);
